  ...
  module Index = struct
    ...
    let get ?(teacher=false) () =
      ...
      if boolX && teacher  then (* Si le dossier X n'existe pas, il n'y a pas de token teacher *)
        Lwt.return([] : Learnocaml_data.Token.t list)
      else
        scan (fun d acc ->
            let d =
            ...
            let stok = String.map (function '/' | '\\' -> '-' (* Remplace les slash par "-" *)
                                            | c -> c) d
            in
            try Lwt.return (Token.parse stok :: acc) (* Token.parse renvoie une liste avec les éléments du token *)
            with Failure _ -> Lwt.return acc
          ) directory []
    
  end (* End module Index *)
  
end (* End module token *)
