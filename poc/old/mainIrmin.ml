...
module Utilisateur = struct
  ...
  let t =
    ...
    record "utilisateur" (fun token login_moodle email token_used password_pfitaxel is_teacher ->
        {token; login_moodle; email; token_used; password_pfitaxel; is_teacher})
    |+ field "token" string (fun t -> t.token)
    |+ field "login_moodle" string (fun t -> t.login_moodle)
    |+ field "login_pfitaxe" string (fun t -> t.email)
...
let user_1 = {
    ...
    login_moodle = "sdszr";
    email = "ezfsef";
    token_used = false; (*Changer le nom *)
...
open Bd
let simple_json =
  Server.respond_string ~status:`OK ~body:(retour) ()
